# Reflection 16 — Osama Aboud (Lab ID: 29)

## Part F Answers

1. Lab ID & Derived Values:
   - Lab ID: 29
   - MAX_YEAR: (29 mod 4) + 1 = 1 + 1 = 2
   - MIN_GPA: 2.5 + (29 mod 3) * 0.5 = 2.5 + 2 * 0.5 = 3.5
   - INTAKE_CODE: itiC (29 mod 3 = 2 -> C)
...
2. Request Flow for `/students/top/5`:
   ...
   - It matches the pattern `students/top/{count:int:range(1,2)}`, but the constraint `range(1,2)` fails because `5` exceeds MAX_YEAR (2).
...
3. Custom vs Built-in Constraint Comparison:
   ...
   - Different: Built-in `int` constraint is provided natively by the framework in `Microsoft.AspNetCore.Routing.Constraints`, whereas `IntakeCodeConstraint` is custom C# code written to validate application-specific logic (`itiC`).
