// LAB 17 — Lab ID: 07 | MIN_GPA_EDIT = 2.6 | MAX_YEAR_EDIT = 3
// Create()'s GET overload is not async because it only renders an empty form and does not perform any database queries.
// =====================================================================
...
namespace StudentPortalWeb.Controllers
{
    public class StudentsController : Controller
    {
        ...
        // This action loads the student from the database so the form can be pre-filled with the existing data for editing.
        [HttpGet]
        ...
        // If the parameter is renamed to studentId while the route expects {id}, model binding will fail to map the URL value, resulting in studentId being 0.
        [HttpPost]
        public async Task<IActionResult> Edit(int id, Student student)
        {
            ...
            return RedirectToAction(nameof(OsamaConfirmed), new { id = student.Id });
        }

        // The action must reload the student from the database to ensure the view displays the actual persisted state, not just the potentially unverified in-memory data.
        public async Task<IActionResult> OsamaConfirmed(int id)
        {
            var student = await _context.Students.FindAsync(id);
            if (student == null)
            {
